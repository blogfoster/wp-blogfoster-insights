 ...
 * @subpackage Blogfoster_Tt/admin
 */
...
class Blogfoster_Tt_Admin {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_slug, plugin_dir_url( __FILE__ ) . 'css/blogfoster-tt-admin.css', array(), $this->plugin_version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_slug, plugin_dir_url( __FILE__ ) . 'js/blogfoster-tt-admin.js', array( 'jquery' ), $this->plugin_version, false );
	...
	public function display_activation_message () {
		...
		$msg  = sprintf( __( 'Welcome to %s! You can find the plugin at %s.', 'blogfoster-tt' ), $this->plugin_name, $link );
	...
	public function main() {
		...
		$text = __( 'For blogfoster affiliates: Let count your page views for blogfoster easily.', 'blogfoster-tt' );
		// print options page
		include_once( 'partials/blogfoster-tt-admin-display.php' );
	}
	...
	*/
	public function register_options () {
		 ...
		 * Documentation:
		   ...
		   overwrite values in {} brackets with your choice:
			...
			'{section_name}' => array(
				...
				'headline' => __( '{Section Headline}', 'blogfoster-tt' ),
				'description' => __( '{Section Description}', 'blogfoster-tt' ),
				'options' => array(
					'{option_name_1}' => array(
						# for all form elements:
						'title'   => __( '{Element Label, required}', 'blogfoster-tt' ),
						'desc'    => __( '{Element Description, e.g. advice what to enter, optional}', 'blogfoster-tt' ),
		   ...
		   Example for a selection field with preselected default value:
			'content_alignment' => array(
				'type'    => 'selection',
				'title'   => __( 'Text Alignment', 'blogfoster-tt' ),
				'desc'    => __( 'Select the alignment of the content within the bar', 'blogfoster-tt' ),
				'values'  => array( 'left' => __( 'left-aligned', 'blogfoster-tt' ), 'center' => __( 'centered', 'blogfoster-tt' ), 'right' => __( 'right-aligned', 'blogfoster-tt' ) ),
				'default' => 'center',
			),
		...
		$this->form_structure = array(
			'1st_section' => array(
				'headline' => __( 'Let count easily', 'blogfoster-tt' ),
				'description' => __( 'The blogfoster Traffic Tracker sends every page view of this website to blogfoster.com. Get the website ID as shown in your blogfoster account. After you have entered it the blogfoster Traffic Tracker starts to work.', 'blogfoster-tt' ),
				'options' => array(
					'website_id' => array(
						'type'    => 'textfield',
						'title'   => __( 'Website ID', 'blogfoster-tt' ),
						'desc'    => __( 'Type in the website ID and save it. The ID has to be a positive integer. Otherwise the tracker will not work.', 'blogfoster-tt' ),
					),
				),
			), // end of 1st_section
		); // end of form_structure
